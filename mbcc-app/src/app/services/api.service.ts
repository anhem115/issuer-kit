...
export class APIService {
  ...
  SCHEMA_ID = 'PEqa6MvEjAcAcwQc7D3F9g:2:mbcc.gov:1.0.3';
  ...
  createCredential(connection_id: string, data: any): Observable<any> {
    console.log(`Create proof request: ${JSON.stringify(data)}`);
    const claimData = this.convertData(data);
    const testData = {
      token: 1,
      connection_id: `${connection_id}`,
      claims: claimData,
    ...
    console.log(
      `data to send credential-exchange: ${JSON.stringify(testData)}`
    );
